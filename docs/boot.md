...
Firmware patches write code to RAM and update interrupt vectors and jump tables to invoke that code. Firmware patches are not persisted over power or shutdown cycles.
...
`PATCH_COPY` (`0x06`) copies data from NVM or ROM into RAM. Command arguments are source address, destination address and number of bytes to copy. `PATCH_COPY2` (`0x0A`) is an alias with bit 3 
...
* `PIP` flag to indicate wheter this is the end of the patch
* Destination address for subsequent `PATCH_DATA` commands
* 16 bit CRC for the patch data (loaded so far?).

Except for the very end of a patch, `PATCH_ARGS` is followed by one or more `PATCH_DATA` commands with the data to be stored in RAM. The `CMD` argument of `PATCH_DATA` is `0xE0` and number of valid bytes in the payload (`CMD[2:0]`) and the LSB of the current state of the CRC (`CMD[3]`). 

Except for `PATCH_IMAGE`, all patch commands are encrypted with an XOR and shifting scheme. The decryption routines use memory content (`0x0000-0x07FF`, boot ROM?) as part of the key.

## Patch encryption

The argument stream of the commands `PATCH_COPY`, `PATCH_ARGS` and `PATCH_DATA` are encrypted. 

Decryption consists of XOR operations and shifting. It also includes random XOR with memory below `0x0800`. Either RAM or more likely ROM mapped to that location. 

TODO: Pseudo code or Python script decrypting patch data.

The CRC is also calculated as part of the decryption.
